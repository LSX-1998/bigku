...
import com.lsx.community.annotation.LoginRequired;
...
public class UserController {
    ...
    public String uploadHead(MultipartFile headerImage, Model model){
        if(headerImage==null){
            model.addAttribute("error","您还能有选择图片");
            return "site/setting" ;
        }
    ...
    public String updatePassword (String originalPassword,String newPassword,String confirmPassword,Model model){
         ...
         if(!originalPassword.equals(dbPassword)){
             model.addAttribute("orPasswordMsg","原始密码不正确");
             return "site/setting" ;
         }
         if(!newPassword.equals(confirmPassword)){
             model.addAttribute("coPasswordMsg","两次密码输入不一致");
             return "site/setting" ;
         }
    ...
    public String getProfilePage (@PathVariable("userId") int userId ,Model model){
         ...
         if(hostHolder.getUser()!=null){
              hasFollowed = followService.hasFollowed(hostHolder.getUser().getId(), userId, CommunityConstant.ENTITY_TYPE_USER);
         }
